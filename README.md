...
Yêu cầu: Python 3.12+, Node.js 20+ và một MongoDB instance.
...
API mặc định chạy tại `http://127.0.0.1:8000`; OpenAPI UI ở `/docs`.
...
Web mặc định chạy tại `http://localhost:4200` và dùng cấu hình development để kết nối API local.

## Cấu hình môi trường

- Chỉ sao chép `.env.example` thành `.env` trên máy chạy API.
- Không commit `.env`, credential file, token, private key hoặc connection string.
- Cấu hình database, JWT, storage và AI bằng environment variables của môi trường triển khai.
- Khi build web production, đặt `TESTORA_API_URL` thành public base URL của API. Đây là endpoint công khai, không phải credential.
...
Quét secret trên toàn bộ file được Git theo dõi:
...
Workflow `.github/workflows/ci-cd.yml` chạy khi push hoặc mở pull request:

1. Quét secret trên file được theo dõi.
2. Cài dependency Python và chạy backend tests/compile check.
3. Chạy `npm ci`, Angular unit tests và production build.
4. Chỉ deploy production từ nhánh `main` sau khi toàn bộ job thành công.

Để bật bước deploy, tạo GitHub Actions variable `VERCEL_DEPLOY_ENABLED=true` và ba Actions secrets do Vercel cung cấp: `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. Giá trị secret không xuất hiện trong workflow hoặc log.

## Triển khai Vercel

`vercel.json` build Angular từ monorepo, phục vụ SPA fallback và thêm security headers. FastAPI được triển khai thành một service Python riêng; đặt `TESTORA_API_URL` trong Vercel project trước khi build frontend.

Có thể triển khai thủ công sau khi liên kết project:

```powershell
npx vercel@59.1.3
npx vercel@59.1.3 --prod
```

## Nguyên tắc an toàn repository

- Repository chỉ chứa source code và file cấu hình mẫu.
- Authorization luôn kiểm tra quyền sở hữu tài nguyên ở API.
- Input được giới hạn kiểu, kích thước và định dạng bằng schema.
- Các thao tác quota, submit và rate limit quan trọng dùng cập nhật atomic.
- CI có quyền `contents: read`; deployment secrets chỉ được dùng ở job production.
- CORS và public API origin phải được cấu hình riêng cho từng môi trường.

## Trạng thái kiểm chứng

- Backend unit tests: 4 test cases.
- Frontend unit tests: 2 test cases.
- Angular production build và mobile/desktop visual QA đã được thực hiện.
- API contract hiện có 33 paths trong OpenAPI.
